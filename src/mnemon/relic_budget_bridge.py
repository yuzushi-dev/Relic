...
Cron: relic:budget-bridge, weekly Monday 05:00 Europe/Rome

State file: relic/budget-bridge-state.json
...
STATE_FILE = Path(os.environ.get("RELIC_DATA_DIR") or str(Path(__file__).resolve().parents[1] / "relic")) / "budget-bridge-state.json"
...
def main() -> int:
    ...
    parser = argparse.ArgumentParser(description="Relic Budget Bridge")
