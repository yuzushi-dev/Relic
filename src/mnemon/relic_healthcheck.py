#!/usr/bin/env python3
"""Relic Health Check - FAST version, verifies pipeline is working.

Cron: relic:healthcheck, daily at 04:00 Europe/Rome
...
def check_backup() -> dict:
    ...
    backup_files = sorted(RELIC_DIR.glob("relic_backup_*.db"), reverse=True)
    if not backup_files:
    ...
    try:
        row = db.execute(
            """SELECT
               ...
               FROM observations WHERE created_at >= datetime('now', '-7 days')""",
        ).fetchone()
