"""Relic Memory Context - operational memory layer for agent sessions.

Reads from mnemon.db (hypotheses, traits, facets, entities) and produces
...
context from Relic's analytical model output:
...
Usage:
  python3 -m relic.relic_memory_context [--query "current state"]
  python3 -m relic.relic_memory_context --format json
"""
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Build operational memory context from Relic analytical data."
    )
    parser.add_argument(
