#!/usr/bin/env python3
"""Relic Biofeedback Ingestion - Amazfit Helio Ring via Zepp API.

Pulls nightly health data (sleep stages, HRV, RHR, stress, SpO2, PAI) and
converts readings into relic personality observations (IMP-20–28).

Cron: relic:biofeedback-pull, daily 04:05 Europe/Rome

Usage:
  python3 relic_biofeedback.py [--date YYYY-MM-DD] [--dry-run]
"""
...
EMAIL = os.environ.get("RELIC_ZEPP_EMAIL", "")
PASSWORD = os.environ.get("RELIC_ZEPP_PASSWORD", "")
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Relic Biofeedback Ingestion")
