#!/usr/bin/env python3
"""Relic Extractor - ingests inbox.jsonl and extracts personality signals via LLM.

Cron: relic:extract, every 2 hours
...
from relic_facet_filter import filter_facets_by_query

SCRIPT = "relic_extractor"
...
def ingest_inbox_jsonl() -> int:
    ...
    from relic_db import get_db, ingest_inbox_line
...
def _recover_truncated_signals(json_str: str) -> list[dict[str, Any]] | None:
    ...
    if last_good is not None:
        ...
        _info("relic_extractor", "truncated_json_recovered",
...
def process_signals(signals: list[dict[str, Any]], messages: list[dict[str, Any]]) -> int:
    ...
    from relic_db import add_observation, get_db, categorize_hour, NON_LINEAR_FACETS, get_trait
    from relic_adaptive import TRUST_FACET, adjust_trust_signal_strength
...
def correlate_checkin_replies(messages: list[dict[str, Any]], client: RuntimeClient) -> int:
    ...
    from relic_db import get_db, get_pending_checkins, capture_reply, add_observation
...
def main() -> int:
    import argparse
    from datetime import datetime, timezone
    from relic_db import get_db, get_pending_inbox, mark_processed, get_all_facets
    from relic_adaptive import (
    ...
    parser = argparse.ArgumentParser(description='Relic Extractor')
