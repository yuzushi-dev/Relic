...
Usage:
  python3 relic_biofeedback_gb_ingest.py [--date YYYY-MM-DD] [--dry-run]
"""
...
SCRIPT     = "relic_biofeedback_gb_ingest"
SYNC_DB    = Path(
    os.environ.get("GADGETBRIDGE_DB_PATH")
    or str(Path(__file__).resolve().parents[2] / "media" / "gadgetbridge_sync" / "Gadgetbridge.db")
)
STATE_PATH = Path(os.environ.get("RELIC_DATA_DIR") or str(Path(__file__).resolve().parents[1] / "relic")) / "gb_last_mtime.txt"
...
def run(local_date: str, dry_run: bool = False) -> None:
    ...
    from relic_biofeedback_gadgetbridge import run as gb_run
    try:
