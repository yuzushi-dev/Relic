...
Cron: relic:passive-scan, every 6 hours
"""
...
SCRIPT = "relic_passive_observer"
DEFAULT_RELATIONAL_AGENT_IDS: list[str] = []  # set via RELIC_RELATIONAL_AGENT env var
STATE_FILE = Path(os.environ.get("RELIC_DATA_DIR") or str(Path(__file__).resolve().parents[1] / "relic")) / "passive-observer-state.json"
LOOKBACK_HOURS = 12
...
def process_passive_signals(signals: list[dict[str, Any]], messages: list[dict[str, Any]],
    ...
    from relic_db import add_observation, get_db, categorize_hour, NON_LINEAR_FACETS
...
def check_caps_predictions(session_text: str) -> int:
    ...
    import re
    import sqlite3
    DB_PATH_LOCAL = Path(os.environ.get("RELIC_DATA_DIR") or str(Path(__file__).resolve().parents[1] / "relic")) / "relic.db"
...
def main() -> int:
    ...
    from relic_db import get_all_facets

    parser = argparse.ArgumentParser(description='Relic Passive Observer')
