#!/usr/bin/env python3
"""Relic Voicenote Transcriber

Scans media/inbound/ for .ogg and .mp3 files not yet transcribed,
transcribes them with faster-whisper (tiny model, Italian),
and inserts the text into the relic inbox table so the
entity extractor can process them.

Cron: relic:voicenote-transcribe, daily 04:30 Europe/Rome

State: relic/voicenote-transcriber-state.json
...
SCRIPT = "relic_voicenote_transcriber"
...
STATE_FILE = Path(os.environ.get("RELIC_DATA_DIR") or str(Path(__file__).resolve().parents[1] / "relic")) / "voicenote-transcriber-state.json"
...
def main() -> int:
    import argparse
    parser = argparse.ArgumentParser(description="Relic Voicenote Transcriber")
