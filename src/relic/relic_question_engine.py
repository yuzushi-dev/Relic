#!/usr/bin/env python3
"""Relic Question Engine - facet-aware check-in question selection.

Scores 60 personality facets using the Facet Gap Score (FGS) formula:
...
Falls back to legacy 5-topic scorer if Relic DB is unavailable.
"""
...
LIGHT_CATEGORIES = {"aesthetic", "communication", "temporal", "cognitive", "language"}
...
def run_relic_scoring(state: dict[str, Any], now: datetime, apply: bool = False,
                           state_path: Path | None = None) -> dict[str, Any]:
    """Main scoring path using Relic DB."""
    from relic_db import get_db, get_all_facets, get_all_traits
    ...
    if apply and state_path:
        # Update relic-specific state
        sk_cfg = state.setdefault("relic", {})
...
def run_legacy_fallback(state: dict[str, Any], now: datetime, apply: bool = False,
    ...
    spec = importlib.util.spec_from_file_location(
        "relic_topic_gap_score",
        Path(__file__).parent / "relic_topic_gap_score.py",
    )
    ...
    sys.modules["relic_topic_gap_score"] = mod
    spec.loader.exec_module(mod)

    import os as _os
    _data_dir = Path(_os.environ.get("RELIC_DATA_DIR", "")) if _os.environ.get("RELIC_DATA_DIR") else Path(__file__).resolve().parents[3] / "runtime"
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Relic Question Engine")
    ...
    # Try Relic engine, fall back to legacy
    try:
        output = run_relic_scoring(state, now, apply=args.apply, state_path=state_path)
        # Log to stderr to keep stdout clean JSON
        import sys as _sys
        print(json.dumps({"level": "info", "script": SCRIPT, "event": "scoring_complete",
                          "engine": "relic", "status": output["status"],
                          "selected": output.get("selected_facet")}), file=_sys.stderr)
    except Exception as e:
        warn(SCRIPT, "relic_fallback", error=str(e))
        try:
