#!/usr/bin/env python3
"""Relic Web UI - monitoring dashboard for the CPIS pipeline.

Requires the webui optional dependencies:
  pip install -e ".[webui]"

Usage:
  python -m relic.webui [--port 8765] [--host 127.0.0.1]
"""
...
HTML_PATH = SCRIPT_DIR / "relic_webui.html"

def _resolve_db_path() -> Path:
    data_dir = _os.environ.get("RELIC_DATA_DIR", "")
    if data_dir:
...
FAVICON_PATH = SCRIPT_DIR.parents[2] / "docs" / "relic.svg"
...
def _load_provider():
    """Return a RelicMemoryProvider backed by the active DB_PATH.

    In OSS this is the demo DB (or RELIC_DATA_DIR).
    ...
    return RelicMemoryProvider(db=get_db())


def _subject_id() -> str:
    return _os.environ.get("RELIC_SUBJECT_ID", "demo-subject")
