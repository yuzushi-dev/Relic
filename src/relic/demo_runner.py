"""Synthetic demo pipeline for the Relic OSS repo.
...
from .relic_db import SCHEMA_SQL as _DEMO_DB_SCHEMA
...
# Schema is imported from relic_db.SCHEMA_SQL (single source of truth).



def _write_demo_db(output_dir: Path, seed: dict, observations: list[dict]) -> None:
    """Write a demo relic.db so the webui can serve synthetic data."""
    db_path = output_dir / "relic.db"
    if db_path.exists():
        db_path.unlink()
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Run the synthetic Relic OSS demo pipeline."
    )
    parser.add_argument(
        "--output-dir",
        type=Path,
