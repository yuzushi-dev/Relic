#!/usr/bin/env python3
"""Relic Portrait Synthesizer - Livello 4

Monthly synthesis of all relic layers into a coherent
narrative portrait of the subject, stored as PORTRAIT.md.

Integrates:
  - 60 personality facets (traits)
...
Output: relic/PORTRAIT.md - injected into relational agent sessions via the bootstrap hook.

Cron: relic:portrait, monthly (1st day, 06:00 Europe/Rome)

Usage:
  python3 relic_portrait.py [--model ...] [--force] [--dry-run]
"""
...
def load_portrait_data(db) -> dict:
    """Load all relevant relic data for portrait synthesis."""
    ...
    return {
        ...
        "verbal_complexity": fetch_verbal_complexity(db),
...
VERBAL_COMPLEXITY_FACET = "language.verbal_complexity"
VERBAL_COMPLEXITY_MIN_CONFIDENCE = 0.30


def fetch_verbal_complexity(db) -> dict | None:
    """Return the language.verbal_complexity trait row, independent of the
    top-25 slice used for the global trait list. Returns None when the
    trait is absent."""
    row = db.execute(
        """
        SELECT f.id, f.spectrum_low, f.spectrum_high,
               t.value_position, t.confidence
        FROM traits t JOIN facets f ON t.facet_id = f.id
        WHERE f.id = ?
        LIMIT 1
        """,
        (VERBAL_COMPLEXITY_FACET,),
    ).fetchone()
    if row is None:
        return None
    if hasattr(row, "keys"):
        return dict(row)
    # Fallback for connections without row_factory set.
    return {
        "id": row[0],
        "spectrum_low": row[1],
        "spectrum_high": row[2],
        "value_position": row[3],
        "confidence": row[4],
    }


def format_verbal_complexity_line(trait: dict | None) -> str | None:
    """Render the language.verbal_complexity trait as a single line for the
    Idiolect section. Returns None when the trait is missing, malformed,
    or below the reporting threshold - so callers can skip the line
    cleanly."""
    if not trait:
        return None
    pos = trait.get("value_position")
    conf = trait.get("confidence")
    if pos is None or conf is None:
        return None
    try:
        conf_f = float(conf)
        pos_f = float(pos)
        ...
        return None
    if conf_f < VERBAL_COMPLEXITY_MIN_CONFIDENCE:
        return None
    pole = _pole_label(
        pos_f,
        trait.get("spectrum_low") or "?",
        trait.get("spectrum_high") or "?",
    )
    return f"  Verbal complexity: {pole} | conf={conf_f:.2f}"
...
def build_prompt(data: dict) -> str:
    ...
    # v3: Idiolect summary
    idiolect_lines = []
    verbal_line = format_verbal_complexity_line(data.get("verbal_complexity"))
    if verbal_line:
        idiolect_lines.append(verbal_line)
...
def run(model: str = DEFAULT_MODEL, force: bool = False, dry_run: bool = False) -> None:
    ...
    try:
        ...
        header = (
            f"<!-- Generated by relic_portrait.py on {now_str} -->\n"
...
def main() -> int:
    import argparse
    parser = argparse.ArgumentParser(description="Relic Portrait Synthesizer")
