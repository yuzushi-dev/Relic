#!/usr/bin/env python3
"""Relic SQLite database: schema, seed data, and module API.

Database at: workspace/relic/relic.db
...
SCRIPT = "relic_db"
DB_DIR = Path(os.environ.get("RELIC_DATA_DIR") or str(Path(__file__).resolve().parents[1] / "relic"))
DB_PATH = DB_DIR / "relic.db"
    ...
    if already_exists:
        try:
            ...
            if n == 0:
                import sys
                print(
                    f"[RELIC CRITICAL] DB at {db_path} has 0 observations - "
...
def init_db(path: Path | None = None) -> None:
    ...
    try:
        ...
        # Migration: inquiry team tables (INQUIRY-01)
        conn.execute("""
            CREATE TABLE IF NOT EXISTS inquiry_cases (
                id TEXT PRIMARY KEY,
                hypothesis_id INTEGER NOT NULL REFERENCES hypotheses(id),
                subclaims TEXT NOT NULL DEFAULT '[]',
                det_temporal_weeks INTEGER,
                det_source_types INTEGER,
                det_min_consistency REAL,
                det_passed INTEGER,
                verdict TEXT,
        ...
        conn.execute("""
            CREATE TABLE IF NOT EXISTS inquiry_evidence (
                id TEXT PRIMARY KEY,
                case_id TEXT NOT NULL REFERENCES inquiry_cases(id),
                subclaim_id TEXT,
                kind TEXT NOT NULL,
        ...
        """)
        conn.execute(
            "CREATE INDEX IF NOT EXISTS idx_inquiry_cases_hypothesis ON inquiry_cases(hypothesis_id)"
        )
        conn.execute(
...
def save_inquiry_case(
    case_id: str,
    hypothesis_id: int,
    subclaims: list[dict],
    det_temporal_weeks: int | None,
    det_source_types: int | None,
    det_min_consistency: float | None,
    det_passed: bool,
    ...
    try:
        ...
        c.execute(
            """INSERT INTO inquiry_cases
                 (id, hypothesis_id, subclaims, det_temporal_weeks, det_source_types,
                  det_min_consistency, det_passed, verdict, confidence_delta, rationale,
                  conflict, requires_human, created_at, updated_at)
               VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
               ON CONFLICT(id) DO UPDATE SET
                 subclaims=excluded.subclaims, det_temporal_weeks=excluded.det_temporal_weeks,
                 det_source_types=excluded.det_source_types,
                 det_min_consistency=excluded.det_min_consistency,
                 det_passed=excluded.det_passed, verdict=excluded.verdict,
                 confidence_delta=excluded.confidence_delta, rationale=excluded.rationale,
                 conflict=excluded.conflict, requires_human=excluded.requires_human,
                 updated_at=excluded.updated_at""",
            (case_id, hypothesis_id, json.dumps(subclaims, ensure_ascii=False),
             det_temporal_weeks, det_source_types, det_min_consistency,
             1 if det_passed else 0, verdict, confidence_delta, rationale,
             1 if conflict else 0, 1 if requires_human else 0, now_iso, now_iso),
        )
        if close:
            c.commit()
    finally:
...
def save_inquiry_evidence(
    ...
    try:
        now_iso = datetime.now(timezone.utc).isoformat()
        c.execute(
            """INSERT OR IGNORE INTO inquiry_evidence
                 (id, case_id, subclaim_id, kind, source_agent, source_ref, strength, content, created_at)
               VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)""",
            (ev_id, case_id, subclaim_id, kind, source_agent, source_ref, strength, content, now_iso),
        )
        if close:
            c.commit()
    finally:
...
def get_hypotheses_for_inquiry(
    include_legacy_confirmed: bool = False,
    conn: sqlite3.Connection | None = None,
) -> list[dict[str, Any]]:
    c = conn or get_db()
    try:
        if include_legacy_confirmed:
            rows = c.execute(
                "SELECT * FROM hypotheses WHERE status IN ('unverified','confirmed') ORDER BY confidence DESC"
            ).fetchall()
        else:
            rows = c.execute(
                "SELECT * FROM hypotheses WHERE status = 'unverified' ORDER BY confidence DESC"
            ).fetchall()
        return [dict(r) for r in rows]
    finally:
        if conn is None:
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Relic DB management")
    ...
    if args.command == "record-checkin":
        ...
        try:
            ...
            signal_path = Path(os.environ.get("RELIC_DATA_DIR") or str(Path(__file__).resolve().parents[1] / "relic")) / "pending-checkin.json"
