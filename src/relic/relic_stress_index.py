#!/usr/bin/env python3
"""Relic Stress Index - Indice composito settimanale di stress.
...
Cron: relic:stress-index, settimanale lunedì 06:00 Europe/Rome

Usage:
  python3 relic_stress_index.py [--week YYYY-WW] [--dry-run]
"""
...
def run(week: str | None = None, dry_run: bool = False) -> None:
    ...
    try:
        ...
        if liwc_count == 0:
            warn(SCRIPT, "no_liwc_data", hint="Run relic_liwc.py --all first")
            return
...
def main() -> int:
    import argparse
    p = argparse.ArgumentParser(description="Relic Stress Index")
