#!/usr/bin/env python3
"""Relic Backfill - dump import + profile import + deduplication.
...
Usage:
  python3 relic_backfill.py [--dry-run] [--step 1|2|3]
  python3 relic_backfill.py --dry-run        # preview all steps
  python3 relic_backfill.py                  # run all steps
"""
...
PROFILE_PATH = Path(os.environ.get("RELIC_DEMO_DIR", "demo")) / "profile.seed.json"
SUBJECT_FROM_ID = os.environ.get("RELIC_SUBJECT_ID", "demo-subject")
...
def main() -> int:
    import argparse
    p = argparse.ArgumentParser(description="Relic Backfill")
