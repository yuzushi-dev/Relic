#!/usr/bin/env python3
"""Relic Profile Bridge - syncs trait model to subject_profile.json + PROFILE.md.

Cron: relic:profile-sync, daily at 03:30 (after synthesis)

Maintains backward compatibility with existing operational protocols by mapping
Relic facets to profile categories and preserving all non-relic records.
"""
...
SCRIPT = "relic_profile_bridge"

import os as _os
def _data_dir() -> Path:
    env = _os.environ.get("RELIC_DATA_DIR", "")
    if env:
        return Path(env)
...
# Relic category → profile categoria mapping
...
def sync_profile() -> dict[str, Any]:
    """Sync Relic traits to subject_profile.json."""
    from relic_db import get_db, get_all_traits, NON_LINEAR_FACETS
    ...
    # Index existing relic records by fonte
    sk_records: dict[str, int] = {}  # fonte -> index in records
    for i, rec in enumerate(records):
        fonte = rec.get("fonte", "")
        if fonte.startswith("relic:"):
            sk_records[fonte] = i
    ...
    for trait in traits:
        ...
        if confidence >= 0.3:
            ...
            if fonte in sk_records:
                ...
                records[idx]["relic_facet_id"] = facet_id
                updated += 1
            else:
                # Create new record
...
def generate_profile_md() -> dict[str, Any]:
    ...
    from relic_db import get_db, get_all_traits, get_hypotheses, get_model_summary, get_context_clusters, NON_LINEAR_FACETS
    ...
    total = summary.get("total_facets", 60)
    ...
    for cat in ["cognitive", "emotional", "communication", "relational",
                "values", "temporal", "aesthetic", "meta_cognition", "language"]:
