...
class SharedContinuityTools:
    ...
    def _validate_subject_scope(
        ...
        """Validate that all subject scope fields are present."""
        if not subject_id:
    ...
    def tool_remember_marker(
        ...
        """
        Remember a continuity marker.

        Requires subject confirmation before storing.
        Does not store clinical labels.
        """
        self._validate_subject_scope(subject_id, gumi_instance_id, hermes_profile_id)
    ...
    def tool_correct_marker(
        ...
        Does not store clinical labels in correction.
        """
        self._validate_subject_scope(subject_id, gumi_instance_id, hermes_profile_id)
    ...
    def tool_get_due_followups(
        ...
        """
        Get markers where followup is due and not exhausted.

        Respects pause state and max attempts.
        """
        self._validate_subject_scope(subject_id)
    ...
    def tool_get_recent_markers(
        ...
        Returns only markers with subject_confirmation=True.
        """
        self._validate_subject_scope(subject_id)
