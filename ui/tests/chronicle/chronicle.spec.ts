...
test("chronicle overview renders stats panel", async ({ page }) => {
  await page.goto(`/workbench/subjects/${SUBJECT}/chronicle`);
...
test("events page filters by severity via URL", async ({ page }) => {
  await page.goto(`/workbench/subjects/${SUBJECT}/chronicle/events?severity=warning`);
...
test("events filter control updates URL", async ({ page }) => {
  await page.goto(`/workbench/subjects/${SUBJECT}/chronicle/events`);
...
test("decisions page filters by status", async ({ page }) => {
  await page.goto(
    `/workbench/subjects/${SUBJECT}/chronicle/decisions?validation_status=validated`,
  );
  await expect(page.getByTestId("decisions-list").locator("li")).toHaveCount(5);
});

test("snapshots page shows diff entries", async ({ page }) => {
  await page.goto(`/workbench/subjects/${SUBJECT}/chronicle/snapshots`);
