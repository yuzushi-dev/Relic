# Relic: Deep Personality Modeling System

**Version**: 3.0
**Deployed**: 2026-02-28 (v1.0), 2026-03-14 (v2.0, v3.0)
**Location**: `$RELIC_DATA_DIR` (default: `./runtime/`)
...
The previous system used 5 generic topics (`priorita_reali`, `energia_stress`, `relazione_calibrazione`, `routine_benessere`, `stile_decisionale`) scored by a Topic Gap Score (TGS) formula, and `demo_profile.json` held 12 shallow operational records. Relic replaces the 5-topic scorer with a 60-facet model while maintaining full backward compatibility - the old scorer remains as a fallback, and the profile JSON format is preserved and extended.
...
Telegram messages the configured subject
       ...
       |
       +---> [Cron: relic:checkin] every 30min 9-22h
       |         Uses relic_question_engine to select a facet,
...
**Location**: `$RELIC_DATA_DIR/relic.db` (SQLite, WAL mode, FK enforcement on)
...
The memory layer extends relic.db with five new tables for episodic memory, communication metrics, and decision tracking.
...
**File**: `workspace/hooks/relic-capture/handler.ts`
...
1. **Inbox capture**: Messages are appended to `$RELIC_DATA_DIR/inbox.jsonl` as JSON lines:
   ```json
...
Logs delivery status to `relic/delivery.jsonl`:
```json
...
**Cron**: `relic:extract` - every 2 hours at `:15`
...
### 5.3 Question Engine (`relic_question_engine.py`)

**Called by**: `relic:checkin` cron (every 30 minutes, 9-22h)
**No LLM calls** - pure algorithmic scoring

The question engine replaces the old `relic_topic_gap_score.py`. It scores all 60 facets using the Facet Gap Score formula:
...
**Fallback**: If the SQLite database is inaccessible (corrupted, missing, migration in progress), the engine automatically falls back to the legacy 5-topic scorer by dynamically importing `relic_topic_gap_score.py`. The output includes `"engine": "legacy_fallback"` so downstream consumers know which engine ran.

### 5.4 Personal Check-in Cron (`relic:checkin`)
...
1. **Gate check**: Runs `relic_question_engine.py` without `--apply`. If status is not `ask_now`, replies `NO_REPLY` (no message sent).

2. **Apply**: Runs again with `--apply` to persist the state update (increment `sent_today`, record to history, update `relic` config in state file).
...
### 5.5 Passive Observer (`relic_passive_observer.py`)

**Cron**: `relic:passive-scan` - every 6 hours at `:45`
...
**Offset-based resume**: Tracks the last processed line offset per session file in `relic/passive-observer-state.json`. On each run, only reads new lines since the last offset. State entries older than 7 days are automatically pruned.
...
### 5.6 Synthesizer (`relic_synthesizer.py`)

**Cron**: `relic:synthesize` - daily at 03:00 Europe/Rome
...
### 5.7 Profile Bridge (`relic_profile_bridge.py`)

**Cron**: `relic:profile-sync` - daily at 03:30 Europe/Rome (after synthesis)
**No LLM calls**
...
**JSON sync**: Maps each Relic category to profile categories:
...
- If a record with `fonte: "relic:{facet_id}"` exists -> update its `contenuto`, `confidenza`, `stato`
...
Records with `fonte` not starting with `relic:` are preserved untouched - the 12 original operational records remain intact. The schema version bumps from `1.1` to `1.2`, adding an optional `relic_facet_id` field on relic-sourced records.

**PROFILE.md generation**: Creates a human-readable Markdown file at `$RELIC_DATA_DIR/PROFILE.md` with:
...
### 5.8 Memory Layer (`relic_memory.py`)

**No cron** - invoked via `relic_dump_import.py extract-memory` and `compute-metrics` commands
...
- `relic_memory.get_memory_summary()` - returns counts for entities (by type), episodes (by type), decisions (by domain), relations, and chats with metrics
- `relic_db.get_full_memory_summary()` - combines the personality model summary with memory layer counts in a single dict
...
Added by `relic_migrate_v2.py`. Store outputs of 8 specialized psychological analyzers.
...
| File | Purpose |
|------|---------|
| `relic_db.py` | SQLite schema (personality + memory tables), 60 facet seed data, full module API, memory query helpers, CLI entrypoints |
| `relic_memory.py` | Memory layer: schema, DB helpers, extraction prompt, signal processing, communication metrics, decision coherence, summary |
| `relic_dump_import.py` | Dump parsing, windowing, personality + memory extraction, communication metrics computation, state DB migration |
| `relic_question_engine.py` | FGS scoring, gate checks, legacy fallback, backward-compatible output |
| `relic_extractor.py` | Inbox ingestion, LLM signal extraction, check-in reply correlation |
| `relic_passive_observer.py` | Session transcript scanning, behavioral meta-signal extraction |
| `relic_synthesizer.py` | Trait consolidation, confidence computation, LLM hypothesis generation |
| `relic_profile_bridge.py` | JSON profile sync, PROFILE.md generation |
| `relic_portrait.py` | Full portrait synthesis - integrates all constructs into a narrative PORTRAIT.md |
| `relic_decisions.py` | Decision extraction from inbox for coherence analysis |
| `relic_reply_extractor.py` | Check-in reply → observation extraction |
| `relic_entity_extractor.py` | Entity + relation extraction from inbox |
| `relic_entity_dedup.py` | Entity deduplication (manual utility) |
| `relic_liwc.py` | Weekly LIWC-style psycholinguistic analysis (no LLM) |
| `relic_stress_index.py` | Weekly composite stress index (no LLM) |
| `relic_schemas.py` | Monthly Early Maladaptive Schema detection (Young) |
| `relic_defenses.py` | Monthly defense mechanism detection (Vaillant hierarchy) |
| `relic_goals.py` | Monthly goal architecture extraction (Little/Klinger) |
| `relic_caps.py` | Monthly CAPS situational signature synthesis (Mischel & Shoda) |
| `relic_sdt.py` | Monthly SDT needs assessment (Deci & Ryan) |
| `relic_attachment.py` | Monthly ECR-R attachment analysis per relationship context |
| `relic_narrative.py` | Monthly narrative identity analysis (McAdams) |
| `relic_idiolect.py` | Monthly idiolect fingerprint (programmatic, no LLM) |
| `relic_appraisal.py` | Monthly Lazarus/Scherer emotional appraisal extraction (LLM) |
| `relic_mental_models.py` | Monthly Johnson-Laird mental model analysis (LLM) |
| `relic_dual_process.py` | Monthly Kahneman System 1/2 balance (hybrid: programmatic + LLM) |
| `relic_constructs.py` | Monthly Kelly personal construct extraction (LLM) |
| `relic_migrate_v3.py` | One-shot v3.0 migration (5 tables + 6 facets) |
| `relic_migrate_v2.py` | One-shot v2.0 migration (tables + facets) |
| `relic_healthcheck.py` | System health monitoring |
| `relic_budget_bridge.py` | Financial behavior → personality observation bridge |
| `relic_voicenote_transcriber.py` | Voice note transcription pipeline |
...
| File | Purpose |
|------|---------|
| `relic.db` | SQLite database (WAL mode) |
...
| `relic:entity-extract` | `0 4 * * *` | Memory entity extraction |
| `relic:decisions` | `15 4 * * *` | Decision extraction |
| `relic:voicenote-transcribe` | `30 4 * * *` | Voice note transcription |
| `relic:budget-bridge` | `0 5 * * 1` | Financial behavior bridge (weekly Mon) |
| `relic:memory-metrics` | `30 5 * * 1` | Communication metrics (weekly Mon) |
| `relic:liwc` | `0 3 * * 0` | LIWC psycholinguistic analysis (weekly Sun) |
| `relic:stress-index` | `0 6 * * 1` | Composite stress index (weekly Mon) |
| `relic:schemas` | `0 5 1 * *` | EMS detection (monthly 1st) |
| `relic:goals` | `30 5 1 * *` | Goal architecture extraction (monthly 1st) |
| `relic:sdt` | `0 6 1 * *` | SDT needs assessment (monthly 1st) |
| `relic:portrait` | `0 6 1 * *` | Full portrait synthesis (monthly 1st) |
| `relic:caps` | `30 5 2 * *` | CAPS signatures (monthly 2nd) |
| `relic:attachment` | `0 5 3 * *` | ECR-R attachment analysis (monthly 3rd) |
| `relic:defenses` | `30 5 3 * *` | Defense mechanism detection (monthly 3rd) |
| `relic:narrative` | `0 6 3 * *` | Narrative identity analysis (monthly 3rd) |
| `relic:idiolect` | `0 4 1 * *` | Idiolect fingerprint (monthly 1st) |
| `relic:appraisal` | `30 4 5 * *` | Appraisal theory analysis (monthly 5th) |
| `relic:mental-models` | `0 5 5 * *` | Mental model extraction (monthly 5th) |
| `relic:dual-process` | `30 5 5 * *` | System 1/2 balance (monthly 5th) |
| `relic:constructs` | `0 6 5 * *` | Personal constructs (monthly 5th) |

Total: 27 cron jobs (26 relic + 1 personal-checkin).
...
If the SQLite database is corrupted or unavailable, the question engine falls back to the legacy 5-topic scorer, ensuring check-ins continue working even if Relic has issues.
...
python3 relic_question_engine.py \
...
python3 relic_dump_import.py extract-memory --dry-run --limit 1

# Extract episodic memory (real)
python3 relic_dump_import.py extract-memory --limit 3

# Retry failed memory extractions
python3 relic_dump_import.py extract-memory --retry-failed

# Compute communication metrics from parsed data
python3 relic_dump_import.py compute-metrics

# Compute metrics for specific platform/chat
python3 relic_dump_import.py compute-metrics --platform whatsapp --chat marco

# Status (shows both personality and memory extraction progress)
python3 relic_dump_import.py status
```
...
python3 relic_extractor.py

# Run passive observation manually
python3 relic_passive_observer.py

# Run synthesis manually
python3 relic_synthesizer.py

# Run profile sync manually
python3 relic_profile_bridge.py
```
...
- **Check-in delivery**: The `relic:checkin` cron should show `lastStatus: "ok"`. If it consistently outputs `NO_REPLY`, the FGS threshold (0.15) may be too high, or gate checks are too restrictive.
...
- **Delivery failures**: Check `relic/delivery.jsonl` for `"success":false` entries. Failed deliveries indicate Telegram API issues.
- **Follow-up triggers**: The capture hook logs `[relic-capture] Follow-up agent triggered for exchange {id}` to the gateway log when a reply is detected.
...
All relic crons log to `cron/runs/relic-*.log`:
- `relic-extract.log`
- `relic-passive.log`
- `relic-synthesize.log`
- `relic-profile-sync.log`

Each script emits structured JSON logs via `lib/log.py` with `script: "relic_*"` for filtering.
